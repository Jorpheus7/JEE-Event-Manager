...
var relations = [
	...
	labels: [
	{ position: 60, attrs: { text: { text: 'SQL141019184334890' } }}
	]
}),	
		new joint.dia.Link({
	source: { id: classes.RENCONTRES.id },
...
_.each(relations, function(r) { r.toBack(); });
 ...
 function saveGraph(){
	if(typeof(Storage)!=="undefined")
  	{
		localStorage.jsonRencontresGraph=(JSON.stringify(graph.toJSON()));
  	}
	else
  	{
 ...
 function loadGraph(){
	if(typeof(Storage)!=="undefined")
  	{
		 graph.fromJSON(JSON.parse(localStorage.jsonRencontresGraph));
  	}
	else
  	{
  		document.getElementById("error").innerHTML="Sorry, your browser does not support web storage...";
